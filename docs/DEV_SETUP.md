...
### 2D/2.5D directional preview

The layered eight-direction character presentation is enabled for preview/profile runs while manual visual acceptance is in progress. Run `make client-2d25d`, or start the client with any `--profile=<name>` such as `--profile=one`, to enable the directional sprite path. Without a profile or `--2d25d`, the existing renderer remains the fallback. For an isolated view of all eight sectors, open `client/tests/sprite_preview.tscn` in Godot and follow `docs/2D_VISUAL_QA.md`.
