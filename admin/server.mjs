...
let releaseCache = { tag: landingSettings.lastReleaseTag || releaseTag, expires: 0 };
...
createServer(async (request, response) => {
  ...
  try {
    ...
    if (request.method === "GET" && url.pathname === "/landing") return send(response, 200, landingPage(await selectedReleaseTag()));
...
function landingPage(activeReleaseTag) {
...
async function selectedReleaseTag() {
  if (landingSettings.releaseMode === "manual") return landingSettings.manualReleaseTag || releaseTag;
  if (releaseCache.expires > Date.now()) return releaseCache.tag;
  try {
    ...
    const requiredAssets = ["deadworld-linux-x86_64.tar.gz", "deadworld-windows-x86_64.zip", "deadworld-android-universal.apk"];
    const releases = await response.json();
    const latest = releases.find((release) => !release.draft && requiredAssets.every((name) => (release.assets || []).some((asset) => asset.name === name)));
...
function defaultLandingSettings() { return { description: "Один постоянный мир. Общие зомби, добыча и последствия. Сервер решает, что произошло на самом деле.", announcement: "", releaseMode: "auto", manualReleaseTag: releaseTag, lastReleaseTag: releaseTag }; }
