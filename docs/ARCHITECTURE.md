...
The repository-root `VERSION` file is the single source of truth for the released tag. `scripts/version.py` derives every platform identifier from it — semantic version, monotonic build number for Android `versionCode` and iOS `CFBundleVersion`, DEB `~prerelease` version, RPM release and artifact filenames — and its `check` mode runs in CI so a tag can never be built from drifted metadata. The tagged release job additionally fails when the git tag does not equal `VERSION`.
