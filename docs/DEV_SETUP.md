...
The layered eight-direction character presentation is currently opt-in while manual visual acceptance is in progress. Run `make client-2d25d` to start the normal client with the directional sprite path enabled. Without `--2d25d`, the existing renderer remains the fallback. For an isolated view of all eight sectors, open `client/tests/sprite_preview.tscn` in Godot and follow `docs/2D_VISUAL_QA.md`.
