...
const releaseTag = process.env.RELEASE_TAG || "v0.1.0-prealpha.7";
...
const coreReleaseAssets = (tag) => [artifact(tag, "linux", "x86_64", "tar.gz"), artifact(tag, "windows", "x86_64", "exe"), artifact(tag, "android", "universal", "apk")];
const releaseAssetGroups = (tag) => [
  { title: "Linux", note: "Portable TAR.GZ", assets: [
    [artifact(tag, "linux", "x86_64", "tar.gz"), "TAR.GZ x86_64"], [artifact(tag, "linux", "arm64", "tar.gz"), "TAR.GZ ARM64"]
  ] },
  { title: "Windows", note: "Standalone EXE", assets: [
    [artifact(tag, "windows", "x86_64", "exe"), "EXE x86_64"], [artifact(tag, "windows", "arm64", "exe"), "EXE ARM64"]
  ] },
...
function landingPage(release) {
  ...
  return html
    .replace('<div class="downloads"><a class="card" href="https://github.com/' + repository + '/releases/download/' + release.tag + '/deadworld-linux-x86_64.tar.gz"><b>Linux</b><p>x86_64 / ARM64 · TAR.GZ</p><span>СКАЧАТЬ TAR.GZ →</span></a><a class="card" href="https://github.com/' + repository + '/releases/download/' + release.tag + '/deadworld-windows-x86_64.exe"><b>Windows</b><p>x86_64 / ARM64 · standalone EXE</p><span>СКАЧАТЬ EXE →</span></a><a class="card" href="https://github.com/' + repository + '/releases/download/' + release.tag + '/deadworld-android-universal.apk"><b>Android</b><p>ARMv7 + ARM64 · release signed APK</p><span>СКАЧАТЬ APK →</span></a></div>', `<div class="downloads">${cards}</div>${checksums}`)
