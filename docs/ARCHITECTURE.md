...
The iOS test pipeline is intentionally unsigned: pinned Godot `4.7.1` exports an Xcode project on a macOS runner, Xcode builds an `iphoneos` arm64 app with signing disabled, and CI packages `Payload/Deadworld.app` as `deadworld-ios-arm64-unsigned.ipa`. Bundle ID is `org.staydev.deadworld`. The preset's `AAAAAAAAAA` Team ID is a syntax-only CI placeholder required by the Godot exporter, not an Apple credential; Xcode receives an empty development team and cannot sign the app. The IPA is only an input for later owner-controlled resigning.
