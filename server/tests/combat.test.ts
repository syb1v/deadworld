...
import { attack, CombatPlayer, parseAttack } from "../src/combat";
import { Zombie } from "../src/zombies";

const player = (inventory: any[]): CombatPlayer => ({ id: "player:test", x: 0, y: 0, health: 100, inventory, lastAttackSequence: -1, nextAttackTick: 0 });
const zombie = (x = 30): Zombie => ({ id: "zombie:test", x, y: 0, vx: 0, vy: 0, hp: 30, state: "IDLE", targetId: "", nextAttackTick: 0, spawnX: x, spawnY: 0, respawnAtTick: 0 });
...
test("pistol consumes server-owned ammo and enforces cooldown", () => {
  const subject = player([{ id: "item:pistol", definitionId: "pistol" }, { id: "item:ammo", definitionId: "pistol_ammo" }]);
  ...
  assert.equal(subject.inventory.some((item) => item.definitionId === "pistol_ammo"), false);
